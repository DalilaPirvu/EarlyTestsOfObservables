...
program Gross_Pitaevskii_1d
  ...
  integer :: sim, temploop, spec = nyq, nTime = nLat*2, minSim = 0, nSims = 200
  integer :: jj, alph = 8

  real(dl), dimension(4) :: lamblist = (/ 0._dl, 0.1_dl, 1._dl, 10._dl /)
  real(dl), dimension(4) :: list_momsq0 = (/ 0.18083783683124693, 1.8844203485190445, 3.731040261915197, 5.597964081807936 /)
  real(dl) :: lambda
  integer :: gam = 0
  ...
  do jj = 1, 4, 1
     ...
     do temploop = 1, 15, 4
         ...
         m2eff = m2bare + 0.5_dl * lambda * list_momsq0(mod(temploop, 4))

         do sim = 0, nSims-1 ! run nSims simulations for the parameters, each with its output files
             call initialise_fields(fld, nyq, phi0, spec, temp, m2eff)
     
             if (sim >= minSim) then
                 call time_evolve(sim, temp, lambda, m2bare, gam)
                 print*, "Simulation ", sim+1, " in ", nSims , "with", lambda, temp, " done!"
             endif
         enddo
     enddo
  enddo
  print*, 'm2eff = ', m2eff, 'm2bare = ', m2bare, 'len = ', len, 'dt = ', dx/alph, 'dx = ', dx, 'dk = ', dk, 'phi0 = ', phi0, 'alph = ', alph, 'spec = ', spec
...
contains
  ...
  subroutine initialise_fields(fld, kmax, phi, klat, temp, m2)
    ...
    call initialize_thermal_fluctuations(fld, len, kmax, phi, klat, temp, m2)
  end subroutine initialise_fields

  subroutine time_evolve(sim, temp, lamb, m2, gam)
    real(dl) :: dt, dtout, temp, lamb, m2
    integer :: i, j, sim, outsize, gam
    ...
    do i = 1, nTime ! this loops over time slices
       call output_fields(fld, dt, dtout, sim, temp, m2)
  ...
  subroutine initialize_thermal_fluctuations(fld, len, km, phi, kc, temp, m2)
    real(dl), dimension(:,:), intent(inout) :: fld
    real(dl), intent(in) :: len, phi, temp, m2
  ...
  subroutine output_fields(fld, dt, dtout, sim, temp, m2)
    real(dl), dimension(1:nLat, 1:2) :: fld
    real(dl) :: dt, dtout, temp, m2
    logical :: o; integer :: m, sim
    integer, parameter :: oFile = 98
    inquire(file='/gpfs/dpirvu/thermal_bubbles/phi4_m2eff_t'//trim(str(nTime))//'_x'//trim(str(nLat))//'_m2'//trim(real_str(m2))//'_temp'//trim(real_str(temp))//'_phi0'//trim(real_str(phi0))//'_lamb'//trim(real_str(lambda))//'_sim'//trim(str(sim))//'_fields.dat', opened=o)
    if (.not.o) then
       open(unit=oFile,file='/gpfs/dpirvu/thermal_bubbles/phi4_m2eff_t'//trim(str(nTime))//'_x'//trim(str(nLat))//'_m2'//trim(real_str(m2))//'_temp'//trim(real_str(temp))//'_phi0'//trim(real_str(phi0))//'_lamb'//trim(real_str(lambda))//'_sim'//trim(str(sim))//'_fields.dat')
       write(oFile,*) "# Lattice Parameters dx = ", dx, 'len = ', len
       write(oFile,*) "# Time Stepping parameters dt = ", dt, "dt_out = ", dtout
       write(oFile,*) "# Other Parameters m2 = ", m2, "dk = ", dk
    endif
