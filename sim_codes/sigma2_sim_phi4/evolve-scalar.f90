...
program Gross_Pitaevskii_1d
  ...
  real(dl), dimension(4) :: sigma0sqlist = (/ 0.180837836831247_dl, 1.884420348519045_dl, 3.7310402619151954_dl, 5.5979640818079375_dl /)
  ...
  do jj = 1, size(lamblist)
     ...
     do temploop = 1, 15, 4
         ...
         m2eff = m2bare + 0.5 * lambda * sigma0sqlist(mod(temploop,4)+1)
         temp = temploop * 1._dl
         ...
         do sim = 0, nSims-1 ! run nSims simulations for the parameters, each with its output files
             ...
             if (sim >= minSim) then
                 call time_evolve(sim, step, temp, lambda, gamma, m2bare)
                 print*, "Simulation ", sim+1, " in ", nSims , "with", lambda, temp, " done!"
             endif
         enddo
      enddo
  enddo
  print*, 'm2bare = ', m2bare, 'm2eff = ', m2eff, 'len = ', len, 'dt = ', dx/alph, 'dx = ', dx, 'dk = ', dk, 'phi0 = ', phi0, 'alph = ', alph, 'spec = ', spec
...
contains
  ...
  subroutine time_evolve(sim, step, temp, lamb, gam, m2)
    real(dl) :: dt, dtout, temp, lamb, m2
    integer :: i, j, sim, step, outsize, gam
    ...
    do i = 1, nTime ! this loops over time slices
       call output_fields(fld, dt, dtout, sim, temp, lamb, gam, m2)
  ...
  subroutine output_fields(fld, dt, dtout, sim, temp, lamb, gam, m2)
    real(dl), dimension(1:nLat, 1:2) :: fld
    real(dl) :: dt, dtout, temp, lamb, m2
    integer :: m, sim, gam
    logical :: o
    integer, parameter :: oFile = 98
    inquire(file='/gpfs/dpirvu/thermal_bubbles/phi4_meffPS_t'//trim(str(nTime))//'_x'//trim(str(nLat))//'_gam'//trim(str(gam))//'_m2'//trim(real_str(m2))//'_temp'//trim(real_str(temp))//'_phi0'//trim(real_str(phi0))//'_lamb'//trim(real_str(lamb))//'_sim'//trim(str(sim))//'_fields.dat', opened=o)
    if (.not.o) then
       open(unit=oFile,file='/gpfs/dpirvu/thermal_bubbles/phi4_meffPS_t'//trim(str(nTime))//'_x'//trim(str(nLat))//'_gam'//trim(str(gam))//'_m2'//trim(real_str(m2))//'_temp'//trim(real_str(temp))//'_phi0'//trim(real_str(phi0))//'_lamb'//trim(real_str(lamb))//'_sim'//trim(str(sim))//'_fields.dat')
