...
program Gross_Pitaevskii_1d
  ...
  integer :: sim, spec = nyq, nTime = nLat*10, minSim = 0, nSims = 100
  integer :: step = 1, alph = 8
  real(dl), parameter :: m2bare = 1._dl
  real(dl), parameter :: m2eff = 0.6441_dl        !0.6441_dl    !m2eff = 0.4087_dl
  real(dl), parameter :: temp = 0.6_dl        !0.6_dl       !temp = 1._dl
  ...
  do sim = 0, nSims-1 ! run nSims simulations for the parameters, each with its output files
      call initialise_fields(fld, nyq, phi0, spec, m2bare, temp)
      if (sim >= minSim) then
...
contains
  ...
  subroutine initialise_fields(fld, kmax, phi, klat, m2, temp)
  ...
  subroutine time_evolve(sim, step, temp, lambda, m2, gam)
    ...
    real(dl), intent(in) :: lambda, m2, temp
  ...
  subroutine output_fields(fld, dt, dtout, sim, temp)
    ...
    inquire(file='/gpfs/dpirvu/thermal_bubbles/phi4_t'//trim(str(nTime))//'_x'//trim(str(nLat))//'_gam'//trim(str(gam))//'_m2'//trim(real_str(m2eff))//'_temp'//trim(real_str(temp))//'_phi0'//trim(real_str(phi0))//'_lamb'//trim(real_str(lambda))//'_sim'//trim(str(sim))//'_fields.dat', opened=o)

    if (.not.o) then
       open(unit=oFile,file='/gpfs/dpirvu/thermal_bubbles/phi4_t'//trim(str(nTime))//'_x'//trim(str(nLat))//'_gam'//trim(str(gam))//'_m2'//trim(real_str(m2eff))//'_temp'//trim(real_str(temp))//'_phi0'//trim(real_str(phi0))//'_lamb'//trim(real_str(lambda))//'_sim'//trim(str(sim))//'_fields.dat')
