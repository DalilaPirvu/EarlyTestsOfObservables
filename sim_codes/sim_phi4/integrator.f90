...
contains
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine gl10(y, dt, lamb, m2, gam) ! y is the vector yvec with fields + time tracker, dt it the time integrated over dx/alph
    real(dl), intent(inout) :: y(1:nVar) !nVar; length of vectors in subroutine derivs
    real(dl), intent(in) :: dt, lamb, m2, gam
    integer :: i, k
...
#ifdef USEBLAS
          call derivs(y + gtmp(:,i)*dt, g(:,i), lamb, m2, gam) ! call Hamilton equations of motion at new time, out comes the new g!!!!
#else
          call derivs(y + g(:,i)*dt, g(:,i), lamb, m2, gam) ! same but different approx
#endif
       enddo
    enddo
#ifdef USEBLAS
