...
program Gross_Pitaevskii_1d
  ...
  integer :: sim, temploop, spec = nyq, nTime = nLat*100, minSim = 0, nSims = 20
...
!    call initialize_vacuum_fluctuations(fld,len,m2eff,kmax,phi,klat)
  ...
  subroutine output_fields(fld, dt, dtout, sim, temp)
    ...
    inquire(file='/gpfs/dpirvu/thermal_bubbles/phi4_gam'//trim(real_str(gam))//'_t'//trim(str(nTime))//'_x'//trim(str(nLat))//'_temp'//trim(real_str(temp))//'_sim'//trim(str(sim))//'_fields.dat', opened=o)
    if (.not.o) then
       open(unit=oFile,file='/gpfs/dpirvu/thermal_bubbles/phi4_gam'//trim(real_str(gam))//'_t'//trim(str(nTime))//'_x'//trim(str(nLat))//'_temp'//trim(real_str(temp))//'_sim'//trim(str(sim))//'_fields.dat')
