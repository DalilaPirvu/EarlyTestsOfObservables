...
program Gross_Pitaevskii_1d
  ...
  do sim = 0, nSims ! run nSims simulations for the parameters, each with its output files
      ...
      if (sim >= minSim) then
          call time_evolve(sim, step, temp, lambda, m2eff)
...
contains
  ...
  subroutine output_fields(fld, dt, dtout, sim, temp, lamb, m2inV)
    ...
    inquire(file='/gpfs/dpirvu/thermal_bubbles/phi4_empirical_potential_with_changed_potential_m2'//trim(real_str(m2inV))//'_t'//trim(str(nTime))//'_x'//trim(str(nLat))//'_temp'//trim(real_str(temp))//'_phi0'//trim(real_str(phi0))//'_lamb'//trim(real_str(lamb))//'_sim'//trim(str(sim))//'_fields.dat', opened=o)
    
    if (.not.o) then
       open(unit=oFile,file='/gpfs/dpirvu/thermal_bubbles/phi4_empirical_potential_with_changed_potential_m2'//trim(real_str(m2inV))//'_t'//trim(str(nTime))//'_x'//trim(str(nLat))//'_temp'//trim(real_str(temp))//'_phi0'//trim(real_str(phi0))//'_lamb'//trim(real_str(lamb))//'_sim'//trim(str(sim))//'_fields.dat')
