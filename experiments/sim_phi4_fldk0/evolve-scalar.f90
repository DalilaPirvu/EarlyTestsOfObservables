...
program Gross_Pitaevskii_1d
  ...
  integer :: sim, temploop, spec = nyq, nTime = 10*nLat, minSim = 250, nSims = 500 
...
contains
  ...
  subroutine initialise_fields(fld,kmax,phi,klat,temp)
    ...
    fld(:,1) = 1._dl ! free field
    fld(:,2) = 0._dl
    yvec(2*nLat+1) = 0._dl ! Add a tcur pointer here

    call initialize_thermal_fluctuations(fld,len,m2eff,kmax,phi,klat,temp)
!    call initialize_vacuum_fluctuations(fld,len,m2eff,kmax,phi,klat)
  end subroutine initialise_fields


  subroutine time_evolve(sim, step, temp, lambda, m2eff, gam)
  ...
  subroutine output_fields(fld, dt, dtout, sim, temp)
    ...
    inquire(file='/gpfs/dpirvu/thermal_bubbles/phi4_fldk0_m2'//trim(real_str(m2eff))//'_gam'//trim(real_str(gam))//'_t'//trim(str(nTime))//'_x'//trim(str(nLat))//'_temp'//trim(real_str(temp))//'_phi0'//trim(real_str(phi0))//'_lamb'//trim(real_str(lambda))//'_sim'//trim(str(sim))//'_fields.dat', opened=o)
    if (.not.o) then
       open(unit=oFile,file='/gpfs/dpirvu/thermal_bubbles/phi4_fldk0_m2'//trim(real_str(m2eff))//'_gam'//trim(real_str(gam))//'_t'//trim(str(nTime))//'_x'//trim(str(nLat))//'_temp'//trim(real_str(temp))//'_phi0'//trim(real_str(phi0))//'_lamb'//trim(real_str(lambda))//'_sim'//trim(str(sim))//'_fields.dat')
    ...
    do m = 1, nLat
       write(oFile,*) fld(m,:), 0.5_dl*gsq(m)
    enddo
