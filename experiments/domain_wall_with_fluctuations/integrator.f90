...
contains
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine gl10(y, dt, lambda, m2eff, gam) ! y is the vector yvec with fields + time tracker, dt it the time integrated over dx/alph
    real(dl), intent(inout) :: y(1:nVar) !nVar; length of vectors in subroutine derivs
    real(dl), intent(in) :: dt, lambda, m2eff, gam
    integer :: i, k
...
#ifdef USEBLAS
          call derivs(y + gtmp(:,i)*dt, g(:,i), lambda, m2eff, gam) ! call Hamilton equations of motion at new time, out comes the new g!!!!
#else
          call derivs(y + g(:,i)*dt, g(:,i), lambda, m2eff, gam) ! same but different approx
#endif
       enddo
    enddo
#ifdef USEBLAS
