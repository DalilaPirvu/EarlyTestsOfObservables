...
program Gross_Pitaevskii_1d
  ...
  integer :: sim, temploop, spec = nyq, nTime = 10*nLat, minSim = 0, nSims = 500 
...
!    call initialize_vacuum_fluctuations(fld,len,m2eff,kmax,phi,klat)
  ...
  subroutine output_fields(fld, dt, dtout, sim, temp)
    ...
    inquire(file='/gpfs/dpirvu/thermal_bubbles/phi4_momk0_m2'//trim(real_str(m2eff))//'_gam'//trim(real_str(gam))//'_t'//trim(str(nTime))//'_x'//trim(str(nLat))//'_temp'//trim(real_str(temp))//'_phi0'//trim(real_str(phi0))//'_lamb'//trim(real_str(lambda))//'_sim'//trim(str(sim))//'_fields.dat', opened=o)
    if (.not.o) then
       open(unit=oFile,file='/gpfs/dpirvu/thermal_bubbles/phi4_momk0_m2'//trim(real_str(m2eff))//'_gam'//trim(real_str(gam))//'_t'//trim(str(nTime))//'_x'//trim(str(nLat))//'_temp'//trim(real_str(temp))//'_phi0'//trim(real_str(phi0))//'_lamb'//trim(real_str(lambda))//'_sim'//trim(str(sim))//'_fields.dat')
